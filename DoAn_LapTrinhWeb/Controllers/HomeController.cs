...
namespace DoAn_LapTrinhWeb.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.KitchenSales = kitchensales;          
            List<Product> fishsales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 10).Take(4).OrderByDescending(item => item.buyturn).ToList();
